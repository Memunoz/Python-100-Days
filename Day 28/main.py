...
def star_countdown():
    ...
    if reps % 8 == 0:
        tittle_label.config(text="Long Break", fg=GREEN)
        count_down(long_break)
    elif reps % 2 == 0:
        tittle_label.config(text="Break", fg=PINK)
        count_down(short_break)
    else:
        tittle_label.config(text="Work", fg=RED)
        count_down(work_sec)
...
start_button = Button(text="Start", highlightthickness=0, command=star_countdown)
